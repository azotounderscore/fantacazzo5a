...
function createSupaStore() {
  ...
  const { data, error } = await sb.auth.signUp({
    ...
    password,
    options: { data: { username, team_name: teamName } }   // 👈 passa i dati al trigger
  });
...
function createSupaStore() {
  ...
  return {
    ...
    async signUp(username, password, teamName) {
      ...
      if (error) throw new Error(error.message);
      if (!data.user) throw new Error('Sessione non creata: disabilita "Confirm email" in Supabase (vedi SETUP.md)');
      const { error: e2 } = await sb.from('profiles').insert({ id: data.user.id, username, team_name: teamName });
      if (e2) throw new Error(e2.message);
      return { id: data.user.id, username, team_name: teamName, is_admin: false, player_id: null };
    },
